...
impl<C: openxr_data::Compositor> vr::IVRInput010_Interface for Input<C> {
    ...
    fn GetSkeletalTrackingLevel(
        ...
        let controller_type = self.get_device_string_property(
            self.get_controller_device_index(*hand).unwrap(),
...
impl<C: openxr_data::Compositor> Input<C> {
    pub fn interaction_profile_changed(&self, session_data: &SessionData) {
        ...
        for device_type in [
            ...
            TrackedDeviceType::Controller { hand: Hand::Left },
            TrackedDeviceType::Controller { hand: Hand::Right },
        ] {
            ...
            let profile_name = match profile_path {
                xr::Path::NULL => {
                    if device.is_some_and(|d| {
                        matches!(d.get_type(), TrackedDeviceType::Controller { .. })
                    }) {
        ...
        for (device_type, profile_path, interaction_profile) in devices_to_create {
            let device = XrTrackedDevice::new(device_type, profile_path, interaction_profile, None);
